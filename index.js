...
function replaceText(word, replacement, text) {
  ...
  const replaceText = text.replace(regex, replacement);
  return replaceText;
}
...
function extractTextInParentheses(str) {
  ...
  for (const match of matches) {
    extractedTexts.push(match[1]); // Додаємо вміст групи (текст в дужках) до масиву
  }
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  const regex = new RegExp(word, "g");
  const replaceStr = str.replace(regex, newWord);
  return replaceStr;
}
